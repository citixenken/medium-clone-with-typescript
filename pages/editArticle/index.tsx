...
const EditArticle: NextPage = () => {
  ...
  const { id } = router.query; //allows us to access the id param in e.g. localhost:3000/article?id=2
  ...
  const editArticle = async () => {
    try {
      ...
      router.push("/article?id=" + id); //see changes after update
    } catch (err: any) {
      alert(err.message);
    }
  };
  ...
  return (
    <Grid.Container gap={1}>
      ...
      <Grid xs={12}>Editing as {user?.email}</Grid>
